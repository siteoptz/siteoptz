...
// Add new tools to the main aiToolsData.json
function addNewToolsToDatabase() {
  console.log('📦 Adding Neil Patel scraped tools to main database...\n');
  ...
  try {
    ...
    // Find the latest new-tools file
    const neilPatelDir = 'data/neil-patel';
    const files = fs.readdirSync(neilPatelDir);
    const newToolsFiles = files.filter(f => f.startsWith('new-tools-')).sort().reverse();
    
    if (newToolsFiles.length === 0) {
      console.log('❌ No new-tools files found. Run the scraper first.');
      return;
    }
    
    const latestFile = path.join(neilPatelDir, newToolsFiles[0]);
    ...
    const summaryFile = path.join(neilPatelDir, `add-summary-${Date.now()}.json`);
