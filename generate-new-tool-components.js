...
/**
 * Generate production components for the 53 new tools from insidr.ai
 * Uses the current aiToolsData.json as the data source
 */
...
function generateComponents() {
  console.log('🔧 Generating production components for new insidr.ai tools...\n');
  ...
  // Load the list of new tools we just added
  const newToolsPath = path.join(__dirname, 'new-tools-to-add.json');
  const newToolBasicData = JSON.parse(fs.readFileSync(newToolsPath, 'utf8'));
  
  // Get the slugs of new tools
  const newToolSlugs = newToolBasicData.map(tool => 
    tool.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')
  );

  // Filter to only the new tools from the database
  const newTools = allTools.filter(tool => newToolSlugs.includes(tool.slug));

  console.log(`📊 Found ${newTools.length} new tools to generate components for\n`);
  ...
  // Generate components for each new tool
  newTools.forEach((tool, index) => {
    try {
      console.log(`📄 [${index + 1}/${newTools.length}] Generating: ${tool.name}`);
  ...
  console.log(`\n🎉 Successfully generated ${successCount}/${newTools.length} components!`);
