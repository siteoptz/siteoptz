...
async function sendEmailWithSendGrid({ to, subject, html, text, from }) {
  try {
    ...
    const msg = {
      to,
      from: from || process.env.EMAIL_FROM || 'info@siteoptz.ai',
      subject,
      text,
      html,
    };
...
async function sendEmailWithSMTP({ to, subject, html, text, from }) {
  try {
    const transporter = getTransporter();
    
    const mailOptions = {
      from: from || `"SiteOptz Team" <${process.env.EMAIL_FROM || 'info@siteoptz.ai'}>`,
      to,
      subject,
      text,
      html,
...
async function sendEmail({ to, subject, html, text, from }) {
  ...
  try {
    ...
    if (useProvider === 'sendgrid' && process.env.SENDGRID_API_KEY) {
      // Try SendGrid first
      result = await sendEmailWithSendGrid({ to, subject, html, text, from });
      
      if (!result.success) {
        console.log('SendGrid failed, falling back to SMTP...');
        result = await sendEmailWithSMTP({ to, subject, html, text, from });
      }
    } else {
